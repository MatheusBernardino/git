		      ...
		      int *nr_checkouts)
{
	 ...
	 * X is created now, the worker could wrongly create the file at X/f, in
	 * the event of a path collision. So postpone the checkout of symlinks.
	 */
	if (S_ISLNK(ce->ce_mode) && !enqueue_symlink_checkout(ce, nr_checkouts))
		return 0;
