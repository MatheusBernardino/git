 ...
 * When --no-set or --set-to-null are used, no tls_test_value should be given.
 ...
 *	     pthread_getspecific() when no value is set.
 *
 * --set-to-null: set NULL to TLS.
 *
...
static void *thread_routine(void *tls_test_value)
{
	...
	if (!no_set) {
		void *stored_value = set_to_null ? NULL : tls_test_value; 
		CHECK_ERR(pthread_setspecific(key, stored_value),
...
static void setup_and_run(char *tls_test_value)
{
	...
	CHECK_ERR(pthread_create(&thread, NULL, thread_routine, (void *)tls_test_value),
...
int cmd__windows_pthread_key(int argc, const char **argv)
{
	...
	if (set_to_null || no_set) {
		...
		die("one (and only one) parameter is accepted after options: the tls_test_value");
	}
