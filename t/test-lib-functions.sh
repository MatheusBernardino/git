...
# This bit is inherited by subdirectories at their creation. We remove it from
# the returning string to prevent callers from having to worry about the state
# of the bit in the test directory.
#
test_modebits () {
	ls -ld "$1" | sed -e 's|^\(..........\).*|\1|' \
			  -e 's|^\(......\)S|\1-|' -e 's|^\(......\)s|\1x|'
