...
test_expect_success 'parallel-checkout with ident' '
	...
	(
		...
		rm A B &&
		GIT_TRACE2="$(pwd)/trace" git reset --hard &&
...
test_expect_success 'parallel-checkout with re-encoding' '
	...
	(
		...
		rm A B &&
		GIT_TRACE2="$(pwd)/trace" git checkout A B &&
		test_workers_in_trace trace 2 &&
...
test_expect_success 'parallel-checkout and external filter' '
	...
	(
		...
		rm A B C *.internal &&
		GIT_TRACE2="$(pwd)/trace" git checkout A B C &&
		test_workers_in_trace trace 2 &&
...
test_expect_success PERL 'parallel-checkout and delayed checkout' '
	...
	parallel_checkout_config 2 0 &&
	GIT_TRACE2="$(pwd)/trace" git -C delayed checkout -f &&
	test_workers_in_trace trace 2 &&
