...
clone_and_check_collision()
{
	...
	then
		# Note: we have to use `core.ignoreCase=0` so that only
		# `file_x` matches the pattern in .gitattributes.
		#
...
# The next tests with filtering don't work on Windows because, on this system,
# collision detection uses strcmp() (when core.ignoreCase=0) to find the
# colliding pair. But they work on OSX, where collision detection uses inode.
