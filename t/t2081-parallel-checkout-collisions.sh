...
test_expect_success CASE_INSENSITIVE_FS 'worker detects basename collision' '
	...
	GIT_TRACE2_EVENT="$(pwd)/trace" git checkout . &&
	test_workers_in_event_trace trace 2 &&
...
test_expect_success CASE_INSENSITIVE_FS 'worker detects dirname collision' '
	...
	# Check that we detected the collisions
...
#
# These two tests make sure that the code is extended to handle these additional
# parallel checkout mechanics.
#
test_expect_success CASE_INSENSITIVE_FS 'collision report on clone (w/ racy file creation)' '
	...
	rm -f trace &&
	parallel_checkout_config 2 0 &&
	GIT_TRACE2_EVENT="$(pwd)/trace" git clone . clone-repo 2>stderr &&
	test_workers_in_event_trace trace 2 &&
...
# core.ignoreCase is false. And we must have this setting so that only 'file_x'
# matches the pattern of the filter attribute. But the test works on OSX, where
# the colliding pairs are found using inode.
#
