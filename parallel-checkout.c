...
#define DEFAULT_THRESHOLD_FOR_PARALLELISM 0
...
static int pc_open(const char *path, void *cb)
{
	struct pc_open_data *data = cb;
...
{
	struct pc_open_data open_data;
	int fd = -1, fstat_done = 0;
	struct strbuf path = STRBUF_INIT;

	open_data.mode = (pc_item->ce->ce_mode & 0100) ? 0777 : 0666;
	 ...
	 */
	if (raceproof_create_file(path.buf, pc_open, &open_data)) {
...
static void send_one_item(int fd, struct parallel_checkout_item *pc_item)
{
	...
	fixed_portion->attr_action = pc_item->ca.attr_action;
...
#define PC_WORKER_BATCH_SIZE 20

static int steal_work(struct pc_worker *this, struct pc_worker *workers,
			   int num_workers)
{
	int i, laziest_id;
	size_t laziest_to_send = 0;

	for (i = 0; i < num_workers; ++i) {
		if (workers[i].nr_to_send > laziest_to_send) {
			laziest_to_send = workers[i].nr_to_send;
			laziest_id = i;
		}
	}

	if (laziest_to_send > 2 * PC_WORKER_BATCH_SIZE) {
		struct pc_worker *laziest = &workers[laziest_id];
		size_t stolen_size = DIV_ROUND_UP(laziest_to_send, 2);
		size_t remaining = laziest_to_send - stolen_size;

		laziest->nr_to_send = remaining;
		this->next_to_send = laziest->next_to_send + remaining;
		this->nr_to_send = stolen_size;
			   ...
			   int num_workers)
{
	...
	sent = to_send_now = PC_WORKER_BATCH_SIZE < this->nr_to_send ?
			     PC_WORKER_BATCH_SIZE : this->nr_to_send;

	this->next_to_complete = this->next_to_send;
	this->nr_to_complete = to_send_now;
...
static struct pc_worker *setup_workers(struct checkout *state, int num_workers)
{
	...
	for (i = 0; i < num_workers; ++i) {
		...
		if (!send_next_batch(worker, workers, num_workers))
